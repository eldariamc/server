...
public class EntityPlayer extends EntityHuman implements ICrafting {
    ...
    public void openFurnace(TileEntityFurnace tileentityfurnace) {
        ...
        this.nextContainerCounter();
        this.playerConnection.sendPacket(new PacketPlayOutOpenWindow(this.containerCounter, 2, tileentityfurnace.getInventoryName(), tileentityfurnace.getSize(), tileentityfurnace.k_(), tileentityfurnace.furnaceSpeed));
        this.activeContainer = container; // CraftBukkit - Use container we passed to event
        this.activeContainer.windowId = this.containerCounter;
        this.activeContainer.addSlotListener(this);
    }
    ...
    @Override
    public void openWaterPipe(TileEntityWaterPipe waterPipe) {
        System.out.println("openWaterPipe() has been called with" + waterPipe);
        // CraftBukkit start - Inventory open hook
        Container container = CraftEventFactory.callInventoryOpenEvent(this, new ContainerWaterPipe(this.inventory, waterPipe));
        System.out.println("container == " + container);
        if (container == null) {
            waterPipe.closeContainer();
            return;
        }
        // CraftBukkit end

        this.nextContainerCounter();
        this.playerConnection.sendPacket(new PacketPlayOutOpenWindow(this.containerCounter, 12, waterPipe.getInventoryName(), waterPipe.getSize(), waterPipe.k_()));
        this.activeContainer = container; // CraftBukkit - Use container we passed to event
        this.activeContainer.windowId = this.containerCounter;
        this.activeContainer.addSlotListener(this);
    }
