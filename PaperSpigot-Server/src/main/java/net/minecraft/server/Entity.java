    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.X) {
            ...
            if (this.W > 0.0F && flag1 && (flag || this.V < 0.05F) && (d6 != d0 || d8 != d2)) {
                ...
                for (blockY = 0; blockY < list.size(); ++blockY) {
                    d1 = ((AxisAlignedBB) list.get(blockY)).b(this.boundingBox, d1);
                }
                ...
                for (blockY = 0; blockY < list.size(); ++blockY) {
                    d0 = ((AxisAlignedBB) list.get(blockY)).a(this.boundingBox, d0);
                }
                ...
                for (blockY = 0; blockY < list.size(); ++blockY) {
                    d2 = ((AxisAlignedBB) list.get(blockY)).c(this.boundingBox, d2);
                }
            ...
            Block block = this.world.getType(blockX, blockY, blockZ);
            ...
            if (d7 != d1) {
                block.b(this.world, blockX, blockY, blockZ, this);
            }
            ...
            if ((this.positionChanged) && (this.getBukkitEntity() instanceof Vehicle)) {
                ...
                org.bukkit.block.Block bukkitBlock = this.world.getWorld().getBlockAt(MathHelper.floor(this.locX), MathHelper.floor(this.locY - (double) this.height), MathHelper.floor(this.locZ));

                if (d6 > d0) {
                    bukkitBlock = bukkitBlock.getRelative(BlockFace.EAST);
                } else if (d6 < d0) {
                    bukkitBlock = bukkitBlock.getRelative(BlockFace.WEST);
                } else if (d8 > d2) {
                    bukkitBlock = bukkitBlock.getRelative(BlockFace.SOUTH);
                } else if (d8 < d2) {
                    bukkitBlock = bukkitBlock.getRelative(BlockFace.NORTH);
                }

                VehicleBlockCollisionEvent event = new VehicleBlockCollisionEvent(vehicle, bukkitBlock);
                this.world.getServer().getPluginManager().callEvent(event);
            }
            // CraftBukkit end

            if (this.g_() && !flag && this.vehicle == null) {
                int j1 = this.world.getType(blockX, blockY - 1, blockZ).b();

                if (j1 == 11 || j1 == 32 || j1 == 21) {
                    block = this.world.getType(blockX, blockY - 1, blockZ);
                }

                if (!(block instanceof BlockLadder)) { // Keyrisium - Generalize to all BlockLadder class
                    d11 = 0.0D;
                }
                ...
                if (this.Q > (float) this.d && block.getMaterial() != Material.AIR) {
                    ...
                    this.a(blockX, blockY, blockZ, block);
                }
            }
