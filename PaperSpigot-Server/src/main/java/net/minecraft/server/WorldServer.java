...
public class WorldServer extends World {
    ...
    public TileEntity getTileEntity(int i, int j, int k) {
        ...
        if (type == Blocks.CHEST || type == Blocks.TRAPPED_CHEST || type == Blocks.ZINC_CHEST || type == Blocks.CRONYXE_CHEST
                || type == Blocks.KOBALT_CHEST) { // Spigot
            if (!(result instanceof TileEntityChest)) {
                result = fixTileEntity(i, j, k, type, result);
            }
        } else if (type == Blocks.FURNACE || type == Blocks.OBSIDIAN_FURNACE) {
